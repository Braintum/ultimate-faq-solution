...
=1.8.2=
* fix(seo): restore FAQ schema output for nested builder content
* 🛠️ Code cleanup and maintenance updates.
...
* Fixed: FAQ group sorting now works as expected.

=1.7.2=
* Improved compatibility: Shortcodes now work when inserted dynamically via ACF fields, popular page builders, and widgets.

=1.7.1=
* Fixed: Page information incorrectly populating inside the FAQ Assistant area.
* Improved: Data handling to ensure only relevant content is displayed in the chatbot assistant interface.

=1.7.0=
* 🛠 Composer dependencies updated for improved stability.
* ➕ Added `exclude_items` parameter to FAQ Group shortcode for more flexible FAQ display.
* 🖼️ Fixed issue with self-hosted video embeds in FAQ answers.
